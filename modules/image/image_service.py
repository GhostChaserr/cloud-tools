...
class ImageService:
    ...
    def scale_img(self, img_source=None, scale_percentage=0.2):
        try:
            image = Image.fromarray(np.uint8(img_source))
            scaled_img = image.resize([int(scale_percentage * s) for s in image.size])

            image_arr = np.array(scaled_img)
            return image_arr
    ...
    def trademark_img(self, trademark_source=None, target_source=None, x_offset=0, y_offset=0, dimensions=[]):
        try:
            ...
            target_img[y_offset:y_offset + trademark_img.shape[0],
            x_offset:x_offset + trademark_img.shape[1]] = trademark_img
